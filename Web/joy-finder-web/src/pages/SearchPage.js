...
class SearchPage extends React.Component {
  ...
  componentDidMount() {
    switch (this.props.data.searchOption) {
      case "city":
        SortEventsService.searchByCity(
        ...
        break;
      case "type":
        SortEventsService.searchByType(
          this.props.data.searchString,
          "ThisWeek"
        ).then((res) => {
          if (res === null) {
        ...
        break;
      case "all":
        SortEventsService.searchAll().then((res) => {
          if (res === null) {
            this.setState({ showNotFound: true, events: null });
          } else {
            this.setState({ events: res.data, showNotFound: false });
        ...
        <div>
          ...
          {this.state.showButtons && (
            <Buttons
              ...
              type="searched"
              searchOption={this.props.data.searchOption}
              searchString={this.props.data.searchString}
            />
          )}
        </div>
      </div>
