  ...
  render() {
    ...
    return (
      <GoogleMap
        ...
        center={{
          lat: parseFloat(this.props.coordinates[0]),
          lng: parseFloat(this.props.coordinates[1]),
        }}
      >
          ...
          this.props.markers.map((marker) => {
            return (
              <Marker
                key={marker.id}
                position={{
                  lat: parseFloat(marker.location.split(",")[0]),
...
class EventDetails extends React.Component {
  ...
  componentDidMount() {
    axios
      .get(`${Const.API_URL}api/events/byId/${this.props.id}`)
      .then((res) => {
        let event = res.data;
        console.log(event);
        this.setState({
  ...
  render() {
    return (
      <div>
        <div className="date-icon-e">
          <MdToday size="10rems" />
        </div>
        <div className="time-icon-e">
          <MdQueryBuilder size="10rems" />
        </div>
        <div className="location-icon-e">
          <MdLocationOn size="10rems" />
        </div>
        ...
        <div className="event-location-e">
          {this.state.street} {this.state.stnumber} {this.state.city}
        </div>
            ...
            this.state.location !== null && (
              <MapWrapAdd
                ...
                coordinates={[
                  ...
                  this.state.location.split(",")[1].split(" ")[1],
                ]}
