...
class SearchByTypePage extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      events: [],
  ...
  async search() {
    switch (this.state.choice) {
      case "past":
        SortService.searchByType(this.state.type, "Past").then((res) => {
          if (res === null) {
            this.setState({
              events: [],
              noEvents: true,
              typeSearched: this.state.type,
            });
          } else {
            this.setState({
              events: res.data,
        ...
        break;
      case "week":
        SortService.searchByType(this.state.type, "ThisWeek").then((res) => {
          if (res === null) {
            this.setState({
              events: [],
              noEvents: true,
        ...
        break;
      case "future":
        SortService.searchByType(this.state.type, "ThisYear").then((res) => {
          if (res === null) {
            this.setState({
              events: [],
              noEvents: true,
  ...
  async pastPressed() {
    SortService.searchByType(this.state.typeSearched, "Past").then((res) => {
      if (res === null) {
        this.setState({
          events: [],
          choice: "past",
  ...
  async weekPressed() {
    SortService.searchByType(this.state.typeSearched, "ThisWeek").then(
      (res) => {
        if (res === null) {
          this.setState({
            events: [],
  ...
  async futurePressed() {
    SortService.searchByType(this.state.typeSearched, "ThisYear").then(
      (res) => {
        if (res === null) {
          this.setState({
            events: [],
    ...
    return (
      <View style={addEventStyle.background}>
        ...
        <Appbar style={{ backgroundColor: "#262733" }}>
          ...
          />
          <Appbar.Content title={`Searched type: ${this.state.typeSearched}`} />
        </Appbar>
